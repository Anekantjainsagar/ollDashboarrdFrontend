...
const ShowModal = ({ setIsOpen, modalIsOpen, clickedTemplate, templateUser }) => {
  ...
  return (
    ...
    >
      <div className="header">
        ...
        <AiOutlineClose onClick={closeModal} style={{cursor: "pointer" }} size={20} />
      </div> 
      <div className="body">
          ...
          (clickedTemplate.customParams.length > 0)?(
            clickedTemplate.customParams.map((e,index)=>{
              return <div className="input">
                  <p className="heading">{index+1+".  " + e.paramName + " :"}</p>
                  <input type="text" />
              </div>
            })
          ):null
        ...
        <button className="btn" onClick={()=>{
          ...
          fetch(`https://live-server-11507.wati.io/api/v1/sendTemplateMessage?whatsappNumber=${templateUser.cCode+templateUser.phone}`, options)
            .then(response => response.json())
            .then((res) => {
              console.log(res)
              const notify = () =>
                toast((res.result?`Message send successfully to the ${templateUser.cCode+templateUser.phone}`:res.info), {
